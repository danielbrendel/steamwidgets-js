             ...
             if (req.readyState == XMLHttpRequest.DONE) {
                 ...
                 let html = `
                     <div class="steam-card" ` + ((cardStyle.length > 0) ? cardStyle: '') + `>
                         <div class="steam-card-image" style="background-image: url('` + json.data.header_image + `'); ` + ((height !== null) ? 'height: ' + height + 'px;' : '') + ((cardImageStyle.length > 0) ? cardImageStyle : '') + ((styleHideImage) ? 'display: none;' : '') + `"></div>
